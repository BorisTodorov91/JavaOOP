package reflaction.P03HighQualityMistakes;

import reflaction.Reflection;
...
import static reflaction.ReflectionUtils.*;
...
public class Main {
    public static void main(String[] args) {
        ...
        TreeSet<Field> getters = collectByName(filterMembersByName(methods, "get"));

        filterMembers(getters.stream(), g -> !Modifier.isPublic(g.getModifiers()))
                .forEach(g -> System.out.println(g.getName() + " have to be public!"));

        TreeSet<Field> setters = collectByName(filterMembersByName(methods, "set"));
